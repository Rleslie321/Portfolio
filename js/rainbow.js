...
const getNextParticleMountPos = () => {
    //added window.scrollY to offsetY to account for scrolling of the page
    const offsetX = IMAGE.getBoundingClientRect().left,
        offsetY = IMAGE.getBoundingClientRect().top + window.scrollY,
...
window.onmousemove = _.throttle(e => {
    if(!STATE.IMAGE.COMPLETE){      //added to make the page not use so much power
        initializeMouseParticleEffect(e.pageX, e.pageY) //changed from clientX, clientY to pageX, pageY to fix scrolling issue
    }
}, 16.66)
